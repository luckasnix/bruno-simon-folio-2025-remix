...
export class Snow {
  ...
  setGeometry() {
    ...
    for (let xIndex = 0; xIndex < this.subdivisions; xIndex++) {
      for (let zIndex = 0; zIndex < this.subdivisions; zIndex++) {
        ...
        const pivotX =
          (xIndex / (this.subdivisions - 1) - 0.5) * (this.size - this.subdivisionSize);
        const pivotZ =
          (zIndex / (this.subdivisions - 1) - 0.5) * (this.size - this.subdivisionSize);
  ...
  setMaterial() {
    ...
    this.material.positionNode = Fn(() => {
      ...
      this.game.terrain.colorNode(terrainData);
  ...
  update() {
    ...
    if (this.elevation.value > -0.9) {
      ...
      this.roundedPosition.value.x =
        Math.round(this.game.view.optimalArea.position.x / this.subdivisionSize) *
        this.subdivisionSize;
      this.roundedPosition.value.y =
        Math.round(this.game.view.optimalArea.position.z / this.subdivisionSize) *
        this.subdivisionSize;
