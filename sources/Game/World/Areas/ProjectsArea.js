...
import { color, float, Fn, If, luminance, mix, step, texture, uniform, uv, vec4 } from "three/tsl";
...
export class ProjectsArea extends Area {
  ...
  setInputs() {
    ...
    this.game.inputs.events.on("forward", (action) => {
      if (action.active && !action.activeKeys.has("Gamepad.RightTrigger")) this.previous();
    });

    this.game.inputs.events.on("backward", (action) => {
      if (action.active && !action.activeKeys.has("Gamepad.LeftTrigger")) this.next();
    });
  ...
  setImages() {
    ...
    this.images.init = (key) => {
      ...
      const colorNode = Fn(() => {
        ...
        uvOld.x.addAssign(
          this.images.animationProgress.mul(0.25).mul(this.images.animationDirection),
        );
  ...
  setDistinctions() {
    ...
    this.distinctions.items.fwa = this.distinctions.group.children.find((_child) =>
      _child.name.startsWith("fwa"),
    );
    ...
    this.distinctions.update = () => {
      ...
      gsap.delayedCall(1, () => {
        ...
        let i = 0;
        const positions =
          this.distinctions.positions[this.navigation.current.distinctions.length - 1];
  ...
  changeProject(
    ...
    this.navigation.previous =
      projectsData[
        this.navigation.index - 1 < 0 ? projectsData.length - 1 : this.navigation.index - 1
      ];
    ...
    if (firstImage) imageIndex = 0;
      ...
      imageIndex =
        direction === ProjectsArea.DIRECTION_NEXT ? 0 : this.navigation.current.images.length - 1;
