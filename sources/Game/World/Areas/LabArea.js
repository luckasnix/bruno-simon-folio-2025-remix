  ...
  setScroller() {
        ...
        // Image
        {
          ...
          mini.startLoading = () => {
            ...
            loader.load(`lab/images/${project.imageMini}`, (loadedTexture) => {
              ...
              material.outputNode = Fn(() => {
                return vec4(
                  mix(baseOutput.rgb, textureColor, this.shadeMix.images.mixUniform),
                  alpha,
                );
              })();
    ...
    this.scroller.animate = () => {
      ...
      for (const mini of this.scroller.minis.items) {
        ...
        mini.intersect.active =
          mini.group.visible &&
          (this.state === LabArea.STATE_OPEN || this.state === LabArea.STATE_OPENING);
    ...
    this.scroller.update = () => {
      ...
      const closestProgress =
        Math.round(
          (this.scroller.progress + this.navigation.index - centeringOffset) / labData.length,
        ) *
          labData.length -
  ...
  setCauldron() {
    ...
    // Heat
    {
      ...
      material.outputNode = Fn(() => {
        ...
        const emissiveColor = mix(color("#ff3e00"), color("#ff8641"), emissiveMix).mul(
          strength.add(1).mul(2),
        );
